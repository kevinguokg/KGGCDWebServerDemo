...
class KGWebServer {
    ...
    private init() {
        ...
        self.webServer.addHandlerForMethod("GET", pathRegex: "^/photos(/)*$", requestClass: GCDWebServerRequest.self) { (request, completionBlock) in
            //completionBlock(GCDWebServerDataResponse(HTML:"<html><body><h1>Testing...</h1></body></html>"))
            print("CASE /photos")
        ...
        self.webServer.addHandlerForMethod("GET", pathRegex: "^/photos/((\\w|\\d)-?)+(/)*$", requestClass: GCDWebServerRequest.self) { (request, completionBlock) in
            ...
            let photoPath = request.path.stringByReplacingOccurrencesOfString("/photos/", withString: "")
            var photoPath2 : String? = nil
            do {
        ...
        self.webServer.startWithPort(8089, bonjourName: "GCD Web Server")
